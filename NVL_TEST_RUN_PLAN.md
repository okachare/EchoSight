...
- EchoSight workspace created at `Deployment/EchoSight_Inference_GUI/`; staged deployment preserved at `Deployment/Test_Run_Detect/`.
...
- The local `Test_Run_Instance_Segmentation` package is an `AnomalyDetection` export with blank task metadata; the GUI now normalizes and renders its `AnomalyResult` mask, label, and score.
...
- EchoSight action buttons now use soft rounded edges and consistent visual states; stale legacy UI branding was removed from active launch/package text.
- The GUI package is now named `Deployment/EchoSight_Inference_GUI/`; entry point `EchoSight.py`, launcher `Launch_EchoSight.bat`, and PyInstaller specs use the EchoSight name.
