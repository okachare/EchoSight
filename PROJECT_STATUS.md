...
| 2026-09-02 | Local inference GUI deployment planning | Complete | Completed; transitioned to active EchoSight development with model loading, TIFF import, inference, results review, and export fully functional. |
| 2026-09-02 | First local EchoSight prototype | Complete | Created `Deployment/EchoSight_Inference_GUI/` with model-folder discovery, common-image and multi-frame TIFF import, background inference progress, Results tab navigation, confidence filtering, and annotated/CSV/JSON export. Model loading and real inference are validated with a local Python 3.9/OpenVINO 2024.5 runtime. |
| 2026-09-02 | Portable installer packaging | Complete | PyInstaller-based standalone executable with all dependencies bundled. Users run `build_installer.ps1` to create portable folder; no Python, OpenVINO, or other external dependencies required on target system. Double-click launch with user-selectable model loading. |
...
| 2026-09-03 | Fully portable standalone installer | Complete | Implemented PyInstaller-based portable packaging with all dependencies (OpenVINO, OpenCV, PIL, NumPy) bundled as wheels into the executable. Users download from git, run `build_installer.ps1`, copy the output folder, and double-click `EchoSight.exe` to launch. No default model loading; users select deployment folders after launch. All systems supported without Python or external dependencies. |
| 2026-09-03 | Button rendering simplification | Complete | Removed complex arc/polygon calculations and replaced with simple rectangle drawing. All buttons now use single 1px border for soft edge effect. Preserved all six color variants (default, run_all, run_current, cancel, apply, preprocess), hover effects, and disabled states. Eliminated shadow artifacts and fold-over issues while maintaining clean, minimal visual appearance. |
| 2026-09-03 | Unified annotation label font size | Complete | Set all detection, classification, and anomaly labels to uniform font size 0.6 (changed detection box labels from 0.5). All annotation types now render consistently across images. Zoom behavior scales appropriately with image zoom. |
...
*Last updated: 2026-09-03 — EchoSight GUI finalized: simplified button rendering (basic rectangles with soft 1px edge), unified annotation label font size to 0.6 across all detection types, fully portable standalone executable with all dependencies bundled (OpenVINO, OpenCV, PIL, NumPy). Users download from git, run `build_installer.ps1`, copy portable folder to any system, and double-click `EchoSight.exe` to launch. No external Python or OpenVINO installation required. Representative multi-TIFF validation and detailed output comparison with Web Geti are next priorities.*
