...
*Last updated: 2026-09-03 — EchoSight development complete: fully portable standalone GUI with mathematically-precise button corners, soft-touch rounded controls, explicit applied preprocessing, per-frame value badges, targeted reset, zoom preservation, independent label/annotation visibility, and comprehensive PyInstaller-based packaging. Users download from git, run `build_installer.ps1`, copy portable folder to any system, and double-click to launch. All dependencies bundled; no external Python or OpenVINO installation required. Representative multi-TIFF validation and output comparison with Web Geti are next priorities.*
