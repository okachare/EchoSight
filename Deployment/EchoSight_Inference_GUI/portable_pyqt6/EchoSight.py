...
# Neutral grey color scheme with pastel semantic colors - warmer tone for cohesion
BACKGROUND = "#272a2f"  # Warmer neutral dark grey (slightly warmer than pure grey)
PANEL = "#343840"       # Corresponding panel color
PANEL_LIGHT = "#3f4450" # Lighter panel for contrast
TEXT = "#e0e0e0"
MUTED = "#999999"
ACCENT = "#808080"
SECONDARY_ACCENT = "#cccccc"
SUCCESS = "#90ee90"  # Pastel green
WARNING = "#ffcc99"  # Pastel orange
DANGER = "#ff9999"   # Pastel red
...
class RoundedFrame(tk.Canvas):
    """Custom frame with rounded corners and optional border."""
    def __init__(self, parent: tk.Misc, bg_color: str = PANEL, border_color: str = "#4a4a4a", border_width: int = 1, radius: int = 8, **kwargs: object) -> None:
    ...
    def _draw_border(self) -> None:
        ...
        self.create_oval(0, 0, r*2, r*2, fill=self.border_color, outline="", tags="border")
        self.create_oval(w-r*2, 0, w, r*2, fill=self.border_color, outline="", tags="border")
        self.create_oval(0, h-r*2, r*2, h, fill=self.border_color, outline="", tags="border")
        self.create_oval(w-r*2, h-r*2, w, h, fill=self.border_color, outline="", tags="border")
        self.create_rectangle(r, 0, w-r, h, fill=self.border_color, outline="", tags="border")
        self.create_rectangle(0, r, w, h-r, fill=self.border_color, outline="", tags="border")

    def get_content_frame(self) -> tk.Frame:
        """Get the inner frame for content placement."""
        if not hasattr(self, "_content_frame"):
            self._content_frame = tk.Frame(self, bg=self.bg_color)
            self.create_window(self.radius + 1, self.radius + 1, window=self._content_frame, anchor="nw", tags="content")
        return self._content_frame


class GroupPanel(tk.Frame):
    """Grouped panel with title, rounded border, and organized content layout."""
    def __init__(self, parent: tk.Misc, title: str = "", bg_color: str = PANEL, border_color: str = "#4a4a4a", **kwargs: object) -> None:
        ...
        # Title label
        if title:
            title_label = tk.Label(self, text=title, bg=bg_color, fg=TEXT, font=("Segoe UI Semibold", 11))
    ...
    def add_widget(self, widget: tk.Widget, row: int, column: int, **grid_kwargs: object) -> None:
        """Add a widget to the content frame using grid layout."""
        if not hasattr(self.content_frame, 'grid_slaves'):
            self.content_frame.columnconfigure(0, weight=1)
        widget.grid(in_=self.content_frame, row=row, column=column, **grid_kwargs)



class RoundedButton(tk.Canvas):
    # Role-based color system: (normal, hover, pressed, border, text)
    ROLE_COLORS = {
        "primary": ("#90ee90", "#a8ffaa", "#78d878", "#6b9d6b", "#1a1a1a"),     # Green
        "secondary": ("#424242", "#505050", "#353535", "#666666", "#e0e0e0"),   # Grey
        "caution": ("#ffcc99", "#ffdd99", "#ffbb66", "#cc9966", "#1a1a1a"),     # Orange
        "destructive": ("#ff9999", "#ffb3b3", "#ff7777", "#cc7777", "#1a1a1a"), # Red
        "utility": ("#424242", "#505050", "#353535", "#666666", "#cccccc"),     # Grey (utility tools)
    }

    def __init__(self, parent: tk.Misc, text: str, command: object, role: str = "secondary", width: int | None = None, **kwargs: object) -> None:
        self.role = role if role in self.ROLE_COLORS else "secondary"
        self.label = text
    ...
    def _draw(self) -> None:
        normal, hover, pressed, border, foreground = self.ROLE_COLORS[self.role]
        ...
        if not self.enabled:
            ...
            color = normal
            text_color = foreground if self.enabled else "#696969"
            border_color = border
        ...
        self.create_oval(0, 0, radius * 2, radius * 2, fill=color, outline=border_color, width=1)
        self.create_oval(canvas_width - radius * 2, 0, canvas_width, radius * 2, fill=color, outline=border_color, width=1)
        self.create_oval(0, canvas_height - radius * 2, radius * 2, canvas_height, fill=color, outline=border_color, width=1)
        self.create_oval(canvas_width - radius * 2, canvas_height - radius * 2, canvas_width, canvas_height, fill=color, outline=border_color, width=1)
        ...
        self.create_line(radius, 0, canvas_width - radius, 0, fill=border_color, width=1)
        self.create_line(0, radius, 0, canvas_height - radius, fill=border_color, width=1)
        self.create_line(radius, canvas_height, canvas_width - radius, canvas_height, fill=border_color, width=1)
        self.create_line(canvas_width, radius, canvas_width, canvas_height - radius, fill=border_color, width=1)
        
        # Draw text centered (with slight offset when pressed)
        text_y = canvas_height // 2 + (1 if self._pressed else 0)
        self.create_text(canvas_width // 2, text_y, text=self.label, fill=text_color, font=self.font, anchor="center")
...
class EchoSightApp(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("CSAM Image Analyser")
    ...
    def _configure_styles(self) -> None:
        ...
        self.style.configure("Panel.TFrame", background=PANEL, relief="flat", borderwidth=1)
        ...
        self.style.map("TButton", background=[("pressed", "#353535"), ("active", "#4a4a4a"), ("disabled", "#3a3a3a")], 
                       foreground=[("pressed", TEXT), ("disabled", "#666666")])
        
        # Input field styling with better disabled state
        self.style.configure("TCombobox", fieldbackground=PANEL, background=PANEL, foreground=TEXT, borderwidth=1, relief="flat", padding=4)
        self.style.map("TCombobox", fieldbackground=[("disabled", "#2a2a2a")], foreground=[("disabled", "#666666")])
        
        # Checkbutton with hover effects
        self.style.configure("TCheckbutton", background=BACKGROUND, foreground=TEXT, font=("Segoe UI", 10), focuscolor="none")
        self.style.map("TCheckbutton", background=[("active", BACKGROUND)], foreground=[("active", SUCCESS), ("disabled", MUTED)])
        self.style.configure("Review.TCheckbutton", background=PANEL, foreground=TEXT, padding=(8, 6), font=("Segoe UI Semibold", 9))
        self.style.map("Review.TCheckbutton", foreground=[("active", SUCCESS), ("disabled", MUTED)], background=[("active", PANEL_LIGHT)])
        
        # Tab styling with hover effect
        self.style.configure("TNotebook", background=BACKGROUND, borderwidth=0, tabmargins=(0, 0, 0, 0), padding=0)
        self.style.configure("TNotebook.Tab", background=PANEL_LIGHT, foreground=MUTED, padding=(16, 10), 
                           font=("Segoe UI Semibold", 10), borderwidth=0, relief="flat")
        self.style.map("TNotebook.Tab", 
                      background=[("selected", SUCCESS), ("active", "#4a4a4a")],  # active = hover
                      foreground=[("selected", "#1a1a1a"), ("active", TEXT)])
        
        # Scale slider styling
        self.style.configure("Horizontal.TScale", troughcolor=PANEL, background=SUCCESS, sliderlength=16, borderwidth=0)
        self.style.map("Horizontal.TScale", background=[("disabled", MUTED)])
        
        # Progress bar styling
        self.style.configure("Horizontal.TProgressbar", troughcolor=PANEL, background=SUCCESS, borderwidth=0, thickness=14)
        self.style.map("Horizontal.TProgressbar", background=[("disabled", MUTED)])
        
        # Treeview styling with hover effect
        ...
        self.style.map("Treeview", background=[("selected", SUCCESS)], foreground=[("selected", "#1a1a1a")])
        self.style.configure("best.Treeview", background=SUCCESS, foreground="#1a1a1a")
        
        # Utility styles (deprecated button styles - kept for compatibility if needed)
        self.style.configure("PreviewBadge.TLabel", background="#1a1a1a", foreground=SUCCESS, padding=(8, 6), font=("Consolas", 8))
    ...
    def _build_analyze_tab(self) -> None:
        ...
        self.load_model_button = RoundedButton(controls, "Load Model", self.load_model, role="secondary", width=100)
        self.load_model_button.grid(row=0, column=0, padx=(0, 10), sticky="w")
        self.import_button = RoundedButton(controls, "Import Images", self.import_images, role="secondary", width=110)
        self.import_button.grid(row=0, column=1, padx=10, sticky="w")
        self.run_button = RoundedButton(controls, "Run Analysis", self.run_all, role="primary", width=115)
        self.run_button.grid(row=0, column=3, padx=5, sticky="e")
        self.run_current_button = RoundedButton(controls, "Run Selection", self.run_current, role="primary", width=115)
        self.run_current_button.grid(row=0, column=4, padx=5, sticky="e")
        self.pause_button = RoundedButton(controls, "Pause", self.pause_run, role="caution", state=DISABLED, width=90)
        self.pause_button.grid(row=0, column=5, padx=5, sticky="e")
        self.cancel_button = RoundedButton(controls, "Cancel", self.cancel_run, role="destructive", state=DISABLED, width=90)
        self.cancel_button.grid(row=0, column=6, padx=(5, 0), sticky="e")
        ...
        self.preprocess_button = RoundedButton(preview_stage, "⚙", self._toggle_preprocess_panel, role="utility", width=3, font=("Segoe UI Symbol", 12))
    ...
    def _build_preprocess_controls(self, parent: ttk.Frame) -> None:
        ...
        RoundedButton(panel, "Close", self._toggle_preprocess_panel, role="secondary", width=6).grid(row=0, column=6, sticky="e", pady=(0, 6))
        ...
        RoundedButton(panel, "Apply Processing", self._apply_preprocessing, role="primary").grid(row=6, column=0, columnspan=5, sticky="ew", padx=(0, 6), pady=(8, 0))
        RoundedButton(panel, "Reset", self._reset_preprocessing, role="secondary").grid(row=6, column=5, columnspan=2, sticky="e", pady=(8, 0))
    ...
    def pause_run(self) -> None:
        """Pause the current operation (can be resumed by running again)."""
        if not self.pause_event.is_set():
            self.pause_event.set()
            self.status_label.configure(text="Operation paused")
        ...
        try:
            while True:
                ...
                if event == "model":
                    ...
                    self._stop_activity()
                    self._set_busy(False)
                    messagebox.showerror("CSAM Image Analyser", value)
        except queue.Empty:
            pass
        except Exception as error:
            self._stop_progress_animation()
            ...
            self._stop_activity()
            self._set_busy(False)
            messagebox.showerror("CSAM Image Analyser", f"The results panel could not update:\n{error}")
        self.after(100, self._poll_worker)
    ...
    def _set_busy(self, busy: bool) -> None:
        ...
        self.pause_button.configure(state=NORMAL if busy else DISABLED, text="Pause")
