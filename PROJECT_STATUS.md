...
| 2026-09-09 | EchoSight Treeview Excel-style sorting | Complete | Replaced results dropdown-based sorting with professional Excel-style clickable column headers in a Treeview widget. Features: 3-column layout (Frame Name \| Confidence % \| Annotation Count), click column header to sort ascending/descending, multi-select for batch export, and visual highlighting of best result. Frame column displays full source filename with frame number suffix (e.g., 'image.png [frame 2]'). Column sorting: Frame by source name, Confidence descending, Annotations descending. Implementation in `Deployment/EchoSight_Inference_GUI/EchoSight.py`; validated and published to both okachare/GeTi_CSAM_PVA (CSAM repo) and okachare/EchoSight (standalone). |
